...
def run_all_models_inference(image_path, output_folder):
    ...
    img_vis = (img_prep * 255).astype(np.uint8)
    ...
    if mask_path.exists():
        ...
        gt_mask = cv2.resize(gt_mask,(img_vis.shape[1], img_vis.shape[0]),interpolation=cv2.INTER_NEAREST)
...
def run_flask_inference(image_path, output_folder):
    Path(output_folder).mkdir(parents=True, exist_ok=True)
    ...
    # Lưu ảnh để hiển thị web
    filename = f"{Path(image_path).stem}_result.png"
    ...
    # lưu json (optional nhưng nên có)
    json_filename = f"{Path(image_path).stem}_summary.json"
    ...
    return {
        ...
        "json_url": f"/static/results/{json_filename}",
        "summary": summary
    }